...
function TestimonialForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name.trim() || !form.quote.trim()) {
      setError(t('testimonials.errNameQuote'))
      return
    }
    setSaving(true)
    setError('')
    try {
      ...
      setError(t('testimonials.errGeneric'))
      setSaving(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">{t('testimonials.customerName')}</label>
        <input
          value={form.name}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">{t('testimonials.quote')}</label>
        <textarea
          value={form.quote}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">{t('testimonials.rating')}</label>
        <Stars rating={form.rating} onChange={(n) => set('rating', n)} />
      </div>

      <div className="flex items-center gap-6">
        <div className="w-28">
          <label className="block text-sm font-medium text-gray-700 mb-1">{t('testimonials.displayOrder')}</label>
          <input
            type="number"
            min="0"
            value={form.order}
        ...
        <div className="flex items-end pb-1">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm font-medium text-gray-700">{t('testimonials.showOnSite')}</span>
          </label>
        </div>
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        >
          {saving ? t('testimonials.saving') : initial ? t('testimonials.saveChanges') : t('testimonials.addBtn')}
        </button>
        <button
          type="button"
          onClick={onCancel}
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-lg font-semibold text-gray-800">{t('testimonials.title')}</h1>
        {!adding && (
          <button
      ...
      {adding && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">{t('testimonials.newTitle')}</h2>
          <TestimonialForm
            onSave={handleAdd}
      ...
      {testimonials.length === 0 && !adding ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
          <p className="text-gray-400 mb-4">{t('testimonials.noTestimonials')}</p>
          <button
            onClick={() => setAdding(true)}
        ...
        <div className="space-y-3">
          {testimonials.map((item) => (
            <div key={item.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm">
              {editingId === item.id ? (
                <div className="p-6">
                  <h2 className="text-sm font-semibold text-gray-700 mb-4">{t('testimonials.editTitle')}</h2>
                  <TestimonialForm
                    initial={item}
                ...
                <div className="p-5 flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1.5">
                      ...
                      <div className="flex gap-0.5">
                        {[1,2,3,4,5].map((n) => (
                          <svg key={n} className={`w-3.5 h-3.5 ${n <= item.rating ? 'text-[#E8B55F]' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                          </svg>
                        ))}
                      </div>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${item.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                        {item.active ? t('testimonials.visible') : t('testimonials.hidden')}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button
                      ...
                      title={item.active ? t('testimonials.hide') : t('testimonials.show')}
                    >
                      {item.active ? (
