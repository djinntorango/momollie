...
export default function Promotions() {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Pop-up Promotions</h1>
        <p className="text-gray-500 text-sm mt-1">Active promotions appear as a pop-up to site visitors</p>
      </div>

      {/* Create form */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-8">
        <h2 className="font-semibold text-gray-700 mb-4">New Promotion</h2>
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-400 py-12">No promotions yet.</p>
      ) : (
          ...
          {promotions.map((promo) => (
            <div key={promo.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                {promo.buttonText && (
                  <p className="text-xs text-gray-400">Button: "{promo.buttonText}" → {promo.buttonLink || '(no link)'}</p>
                )}
              </div>
